...
function generateNum() {
  var $randomnbr = $('.nbr'),
      ...
      lettersToAvoid = [4, 6, 8, 9, 10, 11, 13, 14, 16, 17, 20, 27],
      lettersToExclude = [11],
  ...
  for (i = 0; i < document.getElementById("amount").value; i++) {
    ...
    for (m = 0; m < lettersToExclude.length; m++) {
      if (lettersToAvoid.includes(lettersToExclude[m])) {
        lettersToAvoid.splice(lettersToAvoid.indexOf(lettersToExclude[m]), 1)
      }
    }
    ...
    for (k = 0; k < letters.length; k++) {
      if (lettersToExclude.includes(letters[k])) {
        ...
        probabilityTable.push(1/((letters.length-lettersToExclude.length)*1.52));
      } else {
        probabilityTable.push((1-(1/((letters.length-lettersToExclude.length)*1.52))*lettersToAvoid.length)/(letters.length-lettersToAvoid.length-lettersToExclude.length));
      }
    }
    // Generate a probability table, with lower chance for specific numbers.

    console.log(Math.max.apply(null, probabilityTable) / Math.min.apply(null, probabilityTable.filter(n => n != Math.min.apply(null, probabilityTable))));
